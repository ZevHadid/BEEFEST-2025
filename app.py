...
st.markdown(
    "An interactive dashboard to explore patterns, hotspots, and clusters in Bangladesh crime data."
)
...
months_filter = (
    ...
    else [i for i, name in enumerate(calendar.month_name) if name == selected_month]
)
...
with col3:
    ...
    month_counts["Month"] = month_counts["Month"].apply(
...
with col4:
    ...
    weekday_counts["Weekday"] = pd.Categorical(
        weekday_counts["Weekday"], categories=weekday_order, ordered=True
    )
    st.plotly_chart(
        px.bar(
            weekday_counts.sort_values("Weekday"),
            x="Weekday",
            y="Count",
            color="Weekday",
            title="Crimes by Weekday",
            template="plotly_white",
        ),
        use_container_width=True,
    )
...
if clustering_df.empty:
    ...
    st.plotly_chart(
        px.scatter(
            ...
            y="PCA2",
            color="Cluster",
            hover_data=["incident_division", "crime"],
            template="plotly_white",
        ),
        use_container_width=True,
    )
